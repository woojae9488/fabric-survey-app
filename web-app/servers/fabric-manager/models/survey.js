...
exports.update = async information => {
    const { id, dName, sCreatedAt, survey } = information;

    const networkObj = await network.connect(id);
    const contractRes = await network.invoke(networkObj, 'update', dName, sCreatedAt, survey);
...
exports.remove = async information => {
    ...
    const networkObj = await network.connect(id);
    const contractRes = await network.invoke(networkObj, 'remove', dName, sCreatedAt, id);
